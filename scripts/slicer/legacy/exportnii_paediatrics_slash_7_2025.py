...
def add_slash(rc: str) -> str:
    # nejprve se ujistíme, že máme jen číslice
    digits = rc.replace('/', '')
    # případně můžete kontrolovat délku:
    if len(digits) != 10:
        raise ValueError(f"Neočekávaná délka rodného čísla: {digits!r}")
    # vložíme lomítko po 6. znaku
...
with open('/media/jezdip1/UBURYZEN24_48T1/motol/AZV_PET/normy/all_series_metadata_export_12082025_pje.csv', mode='r') as file:
    ...
    for row in reader:
        ...
        try:
            print(f'trying to load patient by patient id ',add_slash(ID))
            loadedNodeIDs = DICOMUtils.loadPatientByPatientID(add_slash(ID))
...
#            filename = shNode.GetItemAttribute(patientItem, 'DICOM.PatientID')
        ...
        # Save node
            ...
            print("Something wrong with the patient, most probably does not exists in database")
            traceback.print_exc()
